...
class SudokuUI(Frame):
    ...
    def draw_solver(self, board):
        ...
        if not self.solved and self.board_loaded:   # Make sure there is a board loaded AND it hasn't been solved
            ...
            for i in range(1, 10):
                ...
                self.canvas.delete(f"visual_solve{row}{col}", f"selection{row}{col}")
                ...
                if self.sudoku.check_valid(board, i, (row, col)):
                    ...
                    self.update()
                    time.sleep(delay*2)
                    if self.draw_solver(board): # Recursive function, ie the function calls itself
                        return True

            
                    board[row][col] = 0
                    self.canvas.delete(f"visual_solve{row}{col}", f"selection{row}{col}") # Delete if backtracking
                    self.lbl_message_log['text'] = "Backtracking..."

                    self.update()
                    time.sleep(delay*2)

    def delete_visual_solve(self):
        # Delete numbers generated by the solver at draw_solver
    ...
    def draw_answers_command(self):
        ...
        self.canvas.delete(f"selection{self.disp_row}{self.disp_col}", f"visual_solve{self.disp_row}{self.disp_col}")
        self.disp_row, self.disp_col = -1, -1   # Reset selection flags
        self.draw_solver(self.puzzle_board)
    ...
    def disp_solv_num(self, number, x, y, row, col, colour):
        ...
        self.canvas.create_text(
        x,y, text=number,font=text_font, tags=f"visual_solve{row}{col}", fill=colour, 
        )
        ...
        if not self.solved and not self.solving and self.board_loaded:
            ...
            if (MARGIN < mouse_x < WIDTH + MARGIN and MARGIN < mouse_y < HEIGHT + MARGIN):
                ...
                if (row, col) == (self.disp_row, self.disp_col):
                    self.canvas.delete(f"selection{row}{col}", f"visual_solve{row}{col}")
                    self.disp_row, self.disp_col = -1, -1
        ...
        if not self.solved and not self.solving and self.board_loaded:
            ...
            if (self.disp_row >= 0 and self.disp_col >= 0):
                ...
                if event.char in "123456789":
                    ...
                    self.canvas.delete(f"visual_solve{self.disp_row}{self.disp_col}") # Clear out the previous answer
